        ...
        for count2 in range(3,elementinelementSayisi):
            if False == isPeakTime(count2):
                ...
                if TimeC[count2-3]+priority[count1][count2] < limitValue :
                    ...
                    ShiftedValue.append(priority[count1][count2])                   ######hatalııııı
...
def discharge(batteryCurrentCharge,batteryMaxCap,batteryHourlyChargeValues,currentHour,YeniTimeC):
    if SOC(batteryCurrentCharge,batteryMaxCap) > 30: 
        ...
        """elif :
            sebekedenCekilenElektrik=batteryDisChargeCap-batteryCurrentCharge
    """
...
for i in range(3,len(e_source[0])):
    if batteryChargeCap <= e_source[0][2]:
        ...
        #fazla elektriği sat 
        FazlaElektrik[i]=(e_source[0][2]-batteryChargeCap)   #satılacak elektrik listesi
...
for i in range(len(batteryCiH)):
    if batteryCurrentCharge+batteryCiH[i] > batteryPerMax and (i<17 or 22<i):
...
print("1.1:",toplam_TimeC/1000,"\n1.5:",toplam_YeniTimeC/1000)
gündüzFatura=0
puantFatura=0
geceFatura=0
tekzfatura=0
...
for i in range(len(FazlaElektrik)):
    if 6<i<17:
        gündüzFatura=gündüzFatura+YeniTimeC[i-3]-FazlaElektrik[i]
        gündüzFatura2=gündüzFatura2+TimeC2[i-3]
    elif 16<i<22:
        puantFatura=puantFatura+YeniTimeC[i-3]-FazlaElektrik[i]
        puantFatura2=puantFatura2+TimeC2[i-3]
    elif i>21 or i<7:
        geceFatura=geceFatura+YeniTimeC[i-3]-FazlaElektrik[i]
        geceFatura2=geceFatura2+TimeC2[i-3]
    tekzfatura=tekzfatura+YeniTimeC[i-3]-FazlaElektrik[i]
    tekzfatura2=tekzfatura2+TimeC2[i-3]
...
print("Smart triple-time tariff bill:",ToplamFatura,"TL\nSmart single-time tariff bill",tekzfaturatutar,"TL")
print("Standart triple-time tariff:",ToplamFatura2,"TL\nStandart single-time tariff bill:",tekzfaturatutar2,"TL")
